...
-- SELECT * es útil para explorar una tabla y conocer rápidamente todas sus columnas y registros.  
--Si no se necesita absolutamente toda la información, puede traer información innecesaria, consumir más recursos y hacer la consulta menos mantenible.

SELECT * FROM sales;

-- Consulta 2: Selección de columnas específicas para finanzas
-- El equipo de finanzas solo necesita identificar al cliente, el producto y el monto de la operación.

SELECT customer_id,product_id,total_amount FROM sales;

-- Consulta 3: Selección con alias en español para stakeholders
-- Los alias permiten mostrar nombres más claros para usuarios que no conocen los nombres técnicos de la base de datos.
